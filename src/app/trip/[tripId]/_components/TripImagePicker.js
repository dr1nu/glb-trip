...
export default function TripImagePicker({ tripId, destinationCountry, initialImagePath }) {
  ...
  return (
    <div className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm shadow-orange-100/50 space-y-3">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <label className="flex-1 text-sm text-neutral-700">
          ...
          <select
            className="w-full rounded-lg border border-orange-100 bg-white px-3 py-2 text-sm text-neutral-900 shadow-sm outline-none focus:border-orange-300 focus:ring-2 focus:ring-orange-200"
            value={selectedPath}
        ...
        <button
          ...
          className="whitespace-nowrap rounded-lg bg-orange-500 px-4 py-2 text-sm font-semibold text-white shadow shadow-orange-200 transition hover:bg-orange-600 disabled:cursor-not-allowed disabled:bg-orange-200"
        >
          {saving ? 'Saving…' : 'Save selection'}
        </button>
      </div>

      <div className="flex flex-col gap-2 rounded-lg border border-dashed border-orange-200 bg-orange-50 px-3 py-3 text-xs text-[#4C5A6B] sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-semibold text-neutral-800">Need a new image?</p>
          <p>Upload to this country folder and apply automatically.</p>
        </div>
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-white px-3 py-2 text-xs font-semibold text-orange-600 shadow-sm shadow-orange-100 transition hover:text-orange-700">
          <input
            type="file"
            accept="image/*"
            className="hidden"
      ...
      {feedback ? <p className="text-xs text-orange-700">{feedback}</p> : null}
    </div>
  );
}
