        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <InfoCard
              ...
              value={formatBookedFlights(preferences)}
              icon={<IconCircle tone="blue"><PlaneIcon /></IconCircle>}
            />
            <InfoCard
              title="Booked accommodation"
              value={formatBookedAccommodation(preferences)}
              icon={<IconCircle tone="mint"><BedIcon /></IconCircle>}
            />
          </div>
          <InfoCard
...
function ConfirmedTripOverview({
  ...
  const billingContext =
    billingCurrency && billingCurrency !== 'EUR' ? ` (${billingCurrency})` : '';
  const paidLabel = billingPaidAt ? new Date(billingPaidAt).toLocaleString() : null;
      ...
      {!paymentRequired && (contact || preferences) ? (
        <section className="rounded-3xl border border-[#ffd9b3] bg-[#fff7ef] p-5 sm:p-6 space-y-4 shadow-sm shadow-[#ff8a00]/10">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            />
            <Detail label="Day trips" value={formatDayTrips(preferences)} />
            <Detail
              label="Special requests"
              value={preferences?.details || contact?.details || '—'}
              className="sm:col-span-2"
            />
            <Detail label="Booked flights" value={formatBookedFlights(preferences)} />
            <Detail label="Booked accommodation" value={formatBookedAccommodation(preferences)} />
          </dl>
        </section>
      ) : null}
    </>
...
function formatTravelWindow(preferences) {
  ...
  if (preferences.travelWindow === 'flexible') {
    ...
    const days = Number(preferences.flexibleDays) || Number(preferences.rangeDays) || 0;
...
function formatBookedFlights(preferences) {
  if (!preferences) return 'Not provided';
  if (preferences.hasBookedFlights === false) return 'No';
  if (preferences.hasBookedFlights !== true) return 'Not provided';
  const parts = [];
  const departNumber = preferences.departureFlightNumber?.trim();
  const departDate = preferences.departureFlightDate?.trim();
  if (departNumber || departDate) {
    parts.push(
      `Departure${departNumber ? ` ${departNumber}` : ''}${
        departDate ? ` (${departDate})` : ''
      }`
    );
  }
  const returnNumber = preferences.returnFlightNumber?.trim();
  const returnDate = preferences.returnFlightDate?.trim();
  if (returnNumber || returnDate) {
    parts.push(
  ...
  if (parts.length === 0) return 'Yes (details pending)';
  return parts.join(' · ');
}

function formatBookedAccommodation(preferences) {
  if (!preferences) return 'Not provided';
  if (preferences.hasBookedAccommodation === false) return 'No';
  if (preferences.hasBookedAccommodation !== true) return 'Not provided';
  const url = preferences.accommodationUrl?.trim();
  return url ? `Yes (${url})` : 'Yes';
}
